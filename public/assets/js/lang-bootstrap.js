...
class LangBootstrap {
  constructor() {
    console.log("LangBootstrap: Initializing...");
  ...
  resolveInitialLanguage() {
    var _a, _b;
    const fromSearch = this.getSearchLang(
      ((_a = window.location) === null || _a === void 0 ? void 0 : _a.search) ||
        ""
    );
    if (fromSearch) return fromSearch;
    const fromPath = this.getPathLang(
  ...
  applyLanguage(lang) {
    ...
    try {
      const allLinks = document.querySelectorAll(
        'a[href^="/"]:not([data-lang-link]):not([data-no-lang])'
      );
    ...
    try {
      document.dispatchEvent(
        new CustomEvent("langchange", { detail: { lang: normalizedLang } })
      );
    } catch (e) {
      // Ignore errors
    }
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => new LangBootstrap(), {
    once: true,
  });
} else {
  new LangBootstrap();
