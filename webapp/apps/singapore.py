...
df.rename(
    columns={
        ...
        "Local cases not residing in doms MOH report": "Local cases not residing in dorms",
    },
    inplace=True,
)
        ...
        dbc.Container(
            [
                dbc.Row(
                    [
                        dbc.Col(
                            html.H1("COVID-19 in Singapore at a glance"),
                            className="mb-2",
                        )
                    ]
                ),
                dbc.Row(
                    [
                        dbc.Col(
                            html.H6(
                                children="Visualising trends across the different stages of the COVID-19 outbreak in Singapore"
                            ),
                            className="mb-4",
                        )
                    ]
                ...
                dcc.RadioItems(
                    ...
                    options=[
                        {"label": i, "value": i}
                        for i in ["Condensed table", "Full table"]
                    ],
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            dbc.Card(
                                html.H3(
                                    children="Situation Across Different Periods of the Outbreak",
                                    className="text-center text-light bg-dark",
                                ),
                                body=True,
                                color="dark",
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            html.H5(
                                children="Daily COVID-19 cases in Singapore",
                ...
                dcc.Graph(
                    id="graph_by_period", hoverData={"points": [{"x": "11-May"}]}
                ),
                dbc.Row(
                    [
                        dbc.Col(
                            html.H5(
                                children="Breakdown of cases: local vs imported",
                                className="text-center",
                            ),
                            width=6,
                            className="mt-4",
                        ...
                        dbc.Col(
                            html.H5(
                                children="Breakdown of cases: whether residing in dorms",
                                className="text-center",
                            ),
                            width=6,
                            className="mt-4",
                ...
                dbc.Row(
                    [
                        dbc.Col(dcc.Graph(id="local and imported"), width=6),
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {
                            "label": "Total completed isolation",
                            "value": "Total completed isolation",
                        },
                        {
                            "label": "Total discharged from hospital",
                            "value": "Total discharged from hospital",
                        },
                    ],
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            html.H5(
                                children="Situation in local hospitals",
                                className="text-center",
                            ),
                            className="mt-4",
                        )
...
@app.callback(Output("graph_by_period", "figure"), [Input("covid_period", "value")])
def update_graph(covid_period_name):
    ...
    data = [
        go.Scatter(
            x=dff["Date"], y=dff["total"], mode="lines+markers", name="Daily confirmed"
        )
    ]
    layout = go.Layout(
        yaxis={"title": "Cases"},
...
@app.callback(
    [Output("local and imported", "figure"), Output("dorms", "figure")],
...
def update_breakdown(hoverData):
    ...
    fig2 = go.Figure()
    fig2.add_trace(
        go.Bar(x=dff["Date"], y=dff["Daily Local transmission"], name="Local cases")
    )

    fig2.add_trace(
        go.Bar(x=dff["Date"], y=dff["Daily Imported"], name="Imported cases")
    )
    ...
    fig3.add_trace(
        go.Bar(
            x=dff["Date"],
            y=dff["Local cases residing in dorms"],
    ...
    fig3.add_trace(
        go.Bar(
            x=dff["Date"],
            y=dff["Local cases not residing in dorms"],
...
@app.callback(
    ...
    [Input("covid_period", "value"), Input("choose_hospital_situation", "value")],
)
